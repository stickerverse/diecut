...
// fabric.js is loaded from a CDN in index.html
...
const ImagePreview: React.FC<{ imageUrl: string; shape: StickerShape; onClear: () => void; }> = ({ imageUrl, shape, onClear }) => {
    ...
    const imageRef = useRef<any>(null); // fabric.Image
    const shapeOverlayRef = useRef<any>(null); // To store the shape overlay object
    ...
    useEffect(() => {
        ...
        canvas.getObjects().forEach((obj: any) => {
            // Keep shape overlay, remove others
    ...
    useEffect(() => {
        ...
        // Reset any container styles and clipPaths from previous implementations
        if (canvas.wrapperEl) {
        ...
        const padding = 8; // A small padding from canvas edge
        ...
        switch (shape.name) {
            ...
            case 'Rounded Corners':
                shapeObject = new fabric.Rect({
                    ...
                    width: shapeDimension,
                    height: shapeDimension,
                    rx: shapeDimension / 8, // corner radius
                    ry: shapeDimension / 8,
                });
                break;
            case 'Contour Cut':
                // No shape overlay needed for contour cut as it follows the image outline
                break;
        }
        
        if (shapeObject) {
            canvas.add(shapeObject);
            canvas.bringToFront(shapeObject); // Ensure it's on top of the image
